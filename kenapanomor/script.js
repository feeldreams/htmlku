const body = document.querySelector("body");const swalst = Swal.mixin({timer: 2300, allowOutsideClick: false, showConfirmButton: false, timerProgressBar: true, imageHeight: 90,}); audio = new Audio('' + linkmp3.src); ftganti=0;fungsi=0;fungsiAwal=0;deffotostiker=fotostiker.src;
  ...
  document.getElementById("loveIn").onclick = function() {
    if (sudahKlik) {
    if(fungsiAwal==0){
      audio.play();fungsiAwal=1;loveIn.style="transition:all .7s ease;transform:scale(0);opacity:0";
  ...
  async function inipesan(){
    var { value: nama } = await swals.fire({
           title: 'Masukin Nama Kamu 😋💐', input: 'text', confirmButtonText: '💌 Mulai'
       });
  ...
  function kethalo(){new TypeIt("#halo", {strings: ["" + teksWelcome], startDelay: 50, speed: 20, waitUntilVisible: true, afterComplete: function(){halo.innerHTML = teksWelcome;setTimeout(bqmuncul,0);},}).go();}
  ...
  document.getElementById("bq").onclick = function() {
    if(opsLclick==1){
      ...
      otomatis();opsLclick=0;
    }
  }
  function aktiopsL(){opsLclick=1;opsLcheck+=1;}
  function gantiopsL(){}
  ...
  //wallpaper.style="transform: scale(1.5);";
  new TypeIt("#kalimat", {
  strings: ["" + vketik3], startDelay: 1, speed: 25, cursor: true, waitUntilVisible: true, lifelike: true,
  afterComplete: function(){
    kalimat.innerHTML=vketik3;
    setTimeout(aktipesan4,1000);
  },}).go();
  }
  ...
  new TypeIt("#pesan4", {
  strings: ["" + vketik4], startDelay: 1, speed: 25, cursor: true, waitUntilVisible: true, lifelike: true,
  afterComplete: function(){
    pesan4.innerHTML=vketik4;
    //setTimeout(aktipesan5,1200);
    
    setTimeout(() => {
      ptimer.style="position:relative;opacity:1;transform:scale(1);";
      const vvpesan = document.getElementById('pesan4');
      let count = 3;

      const interval = setInterval(() => {
        count--;
        initimer.innerHTML = count;
  ...
  fthilang();ftganti=2;setTimeout(ftmuncul,300);
  new TypeIt("#pesan5", {
  strings: ["" + vketik5], startDelay: 1, speed: 25, cursor: true, waitUntilVisible: true, lifelike: true,
  afterComplete: function(){
    pesan5.innerHTML=vketik5;
    setTimeout(aktipesan6,1800);
  },}).go();
  }
  ...
  new TypeIt("#pesan6", {
  strings: ["" + vketik6], startDelay: 1, speed: 25, cursor: true, waitUntilVisible: true, lifelike: true,
  afterComplete: function(){
    pesan6.innerHTML=vketik6;
    setInterval(berjatuhan,200);
    fthilang();ftganti=4;setTimeout(ftmuncul,300);
    setTimeout(function(){tombol();clearInterval(scrollInterval)},500);
  ...
  fthilang();ftganti=4;setTimeout(ftmuncul,300);
  new TypeIt("#pesan7", {
  strings: ["" + vketik7], startDelay: 1, speed: 33, cursor: true, waitUntilVisible: true, lifelike: true,
  afterComplete: function(){
  ...
  function checkslov() {if(slov==4){kolombaru.style="position:relative;transform:scale(1)";fthilang();ftganti=1;setTimeout(ftmuncul,300);otomatis();setTimeout(()=>{kolombaru.style="";kalimat.innerHTML="";},350);setTimeout(aktipesan3,500);}}
...
const scrollContainer = document.getElementById("bq");
function autoScroll() {
