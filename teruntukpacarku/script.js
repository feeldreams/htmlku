audio = new Audio('' + linkmp3.src);
    ...
    function pindahHal(hal) {
      ...
      setTimeout(()=>{
         if (hal === 2) {mulaiHal2();document.getElementById(`hal${hal}`).style="transform:scale(1);opacity:1;transition:all .7s ease";circgroup.style="opacity:0;transition:all .7s ease"}
         if (hal !== 2){
         document.getElementById(`hal${hal - 1}`).style = "transform:scale(0);opacity:0;transition:all .7s ease";
         setTimeout(()=>{
         	document.getElementById(`hal${hal - 1}`).classList.add('sembunyi');
             document.getElementById(`hal${hal}`).classList.remove('sembunyi');
	         setTimeout(()=>{document.getElementById(`hal${hal}`).style="transform:scale(1);opacity:1;transition:all .7s ease";}, 100);
	         setTimeout(()=>{if (hal === 3) mulaiHal3();circgroup.style="opacity:1;transition:all .7s ease"}, 150);
	     }, 500);
	     }
      }, 50);
    }
